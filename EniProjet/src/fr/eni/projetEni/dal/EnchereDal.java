...
public class EnchereDal {
    ...
    private static final String GET_MAX_BY_UTILISATEUR="SELECT * FROM Encheres WHERE no_utilisateur=? ORDER BY no_article ASC,montant_enchere DESC ";
    ...
    public static List<EnchereBo> getmaxbyutilisateur(int no_Utilisateur)
    {
        ...
        try(Connection cnx = ConnectionProvider.getConnection())
        {
            ...
            int artnex=0;
            	...
            	while(rs.next())
            	{
            		if( artnex!=rs.getInt("no_article") )
            			{
